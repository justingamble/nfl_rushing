defmodule NflRushingWeb.PlayerLive.FilterAndSortComponent do
  ...
  @impl true
  def mount(socket) do
    IO.puts("Inside filter_and_sort_component, here is what socket looks like:")
    IO.inspect(socket, limit: :infinity)
#    parent_params = socket.assigns.parent_params
#    paginate_options = set_pagination_from(parent_params)
...
#        options: paginate_options
  ...
  def update(assigns, socket) do
    IO.puts("** Inside filter_and_sort_component's update(), here is what socket looks like:")
    IO.inspect(socket, limit: :infinity)
    IO.puts("** Inside filter_and_sort_component's update(), here is what assigns looks like:")
    IO.inspect(assigns, limit: :infinity)

    parent_params = socket.assigns.parent_params
    paginate_options = set_pagination_from(parent_params)

#    paginate_options =
#      socket.assigns.parent_params
#      |> set_pagination_from()
    ...
    IO.puts("------------------ filter_and_sort_component: update() ---------------\n")
      ...
      |> push_patch(
        to:
          Routes.live_path(
            socket,
#            __MODULE__,
            NflRushingWeb.PlayerLive.Index,
