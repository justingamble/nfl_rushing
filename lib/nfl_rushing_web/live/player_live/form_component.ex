defmodule NflRushingWeb.PlayerLive.FormComponent do
  ...
  def update(%{player: player} = assigns, socket) do
    changeset = PlayerStats.change_player(player)
  ...
  def handle_event("validate", %{"player" => player_params}, socket) do
    changeset =
      ...
      |> PlayerStats.change_player(player_params)
  ...
  defp save_player(socket, :edit, player_params) do
    case PlayerStats.update_player(socket.assigns.player, player_params) do
      {:ok, _player} ->
        {:noreply,
         socket
  ...
  defp save_player(socket, :new, player_params) do
    case PlayerStats.create_player(player_params) do
      {:ok, _player} ->
        {:noreply,
         socket
